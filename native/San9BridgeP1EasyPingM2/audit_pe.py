#!/usr/bin/env python3
"""Fail-closed static PE audit for the P1 M2a offline self-test."""
...
EXPECTED_TLS_CALLBACK_TABLE_RVA = 0x0001FF60
EXPECTED_PINNED_CRT_TLS_CALLBACKS = (
    {
        "rva": 0x0000B560,
        "section": ".text",
        "code_size": 103,
        "code_sha256": "EF31E41701C5D7C94EB7BFEF1106CE34CD49A28D99841540B9A5E9E07CBC1568",
    },
    {
        "rva": 0x0000B5D0,
        "section": ".text",
...
def section_for_rva(image: pefile.PE, rva: int) -> tuple[str, bool, bool] | None:
...
def verify_pinned_crt_tls_callbacks(image: pefile.PE) -> None:
    ...
    actual_rvas = tuple(
        callback - image.OPTIONAL_HEADER.ImageBase for callback in tls_callbacks(image)
    )
    ...
    for callback_rva, evidence in zip(actual_rvas, EXPECTED_PINNED_CRT_TLS_CALLBACKS):
        section = section_for_rva(image, callback_rva)
        if section is None:
...
def audit(
    ...
    whole_sha256 = hashlib.sha256(raw_image).hexdigest().upper()
    if enforce_whole_image_identity and whole_sha256 != EXPECTED_WHOLE_IMAGE_SHA256:
        fail(
            "whole offline image SHA-256 rejected: "
            f"actual={whole_sha256} expected={EXPECTED_WHOLE_IMAGE_SHA256}"
        )
    ...
    try:
        ...
        entrypoint = section_for_rva(image, entrypoint_rva)
        ...
        if not entrypoint_executable or entrypoint_writable:
            fail(
                ...
                f"rva={entrypoint_rva:#x} section={entrypoint_name!r} "
                f"executable={entrypoint_executable} writable={entrypoint_writable}"
            )
        ...
        if forbidden:
            fail(f"live/process/write/IPC imports rejected: {forbidden}")
    ...
    if emit_pass:
        print(
            "P1_M2A_PE_AUDIT PASS machine=x86 format=PE32 console=1 aslr=1 nx=1 "
            "reloc=1 exports=0 delay_imports=0 pinned_crt_tls_callbacks=2 "
            "tls_rva_code_identity=exact duplicate_import_descriptors=0 "
            "ordinal_imports=0 rwx=0 entry_rx_nonw=1 exact_imports=1 "
            "process_access=0 target_writes=0 ipc=0 live_target_code=0 "
            f"whole_sha256={whole_sha256}"
        )


def expect_fixture_rejected(path: Path, expected_text: str, name: str) -> None:
    try:
        audit(str(path), emit_pass=False, enforce_whole_image_identity=False)
    except RuntimeError as error:
        if expected_text not in str(error):
            fail(f"malicious fixture {name} failed for the wrong reason: {error}")
        return
...
def run_malicious_fixture_selftests(exe_path: Path) -> None:
    ...
    with tempfile.TemporaryDirectory(
        prefix="p1-m2a-malicious-pe-", dir=str(exe_path.parent)
    ) as temporary_root:
        ...
        tls_entrypoint = bytearray(original)
        struct.pack_into("<I", tls_entrypoint, tls_table_offset, entrypoint_va)
        tls_entrypoint_path = root / "tls-entrypoint-substitution.exe"
        tls_entrypoint_path.write_bytes(tls_entrypoint)
        expect_fixture_rejected(
            tls_entrypoint_path, "TLS callback RVA/order changed", "tls-entrypoint"
        )
    ...
    print(
        "P1_M2A_PE_AUDIT_SELFTEST PASS fixtures=5 duplicate_import=reject "
        "tls_entrypoint=reject tls_code_mutation=reject tls_tail_mutation=reject "
        "entrypoint_data=reject"
    )
...
def main() -> int:
    ...
    try:
        ...
        print(f"P1_M2A_PE_AUDIT FAIL {error}", file=sys.stderr)
        return 1
