...
static void transform(San9Sha256Context *context, const uint8_t block[64])
{
    uint32_t words[64];
    uint32_t a;
    uint32_t b;
    ...
    context->state[6] += g;
    context->state[7] += h;
    san9_secure_zero(words, sizeof(words));
}
...
void san9_sha256_finish(San9Sha256Context *context, uint8_t output[32])
{
    ...
    san9_secure_zero(length_bytes, sizeof(length_bytes));
    san9_secure_zero(context, sizeof(*context));
}
    ...
    uint8_t output[32])
{
    ...
    if (key_size > sizeof(normalized)) {
        ...
        san9_sha256_update(&context, key, key_size);
        san9_sha256_finish(&context, normalized);
    } else {
    ...
    san9_sha256_initialize(&context);
    san9_sha256_update(&context, inner_pad, sizeof(inner_pad));
    san9_sha256_update(&context, data, data_size);
    san9_sha256_finish(&context, inner_digest);

    san9_sha256_initialize(&context);
    san9_sha256_update(&context, outer_pad, sizeof(outer_pad));
    san9_sha256_update(&context, inner_digest, sizeof(inner_digest));
    san9_sha256_finish(&context, output);

    san9_secure_zero(normalized, sizeof(normalized));
    san9_secure_zero(inner_pad, sizeof(inner_pad));
    san9_secure_zero(outer_pad, sizeof(outer_pad));
    san9_secure_zero(inner_digest, sizeof(inner_digest));
}
